...
MazeEasy::MazeEasy(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MazeEasy)
    , sec(0)
    , msec(0)
...
void MazeEasy::join(int x,int y)//合并集合
{
...
void MazeEasy::carveway(int x,int y)//在x,y间打开一条路
{
    linking[x][y]=1;//1代表连通
    linking[y][x]=1;
...
void MazeEasy::mazegenerator()//生成迷宫
{
    if(!header.empty())
        header.clear();
    if(!linking.empty())
    ...
    while(find(0)!=find(GRIDNUM-1))//当起点终点无路径时
    {
        ...
        int neibour=selectneibour(random);
...
void MazeEasy::paintEvent(QPaintEvent *event)
{
    QPainter painter(this);
    painter.drawPixmap(QPoint(0, 0), *pixmap);
    if(sec<30&&ui->man->pos()==ui->end->pos())//60内抵达终点则赢
    {
    ...
    else if(sec>=30&&ui->man->pos()!=ui->end->pos())//60秒内未达终点则输
    {
...
void MazeEasy::keyPressEvent(QKeyEvent *event)
{
    if(!success)//失败可重来
    {
        msec=0;sec=0;
